...
- **UI:** Corrección del comportamiento de desplazamiento del menú lateral (`DashboardSidebar`) en el panel principal [layout.tsx](file:///c:/Users/CORP-TI012LT/Code/psi/b_wmDIj71Ethj/app/dashboard/layout.tsx), reestructurando el contenedor con las clases `h-screen overflow-hidden` y el panel de contenido a `overflow-y-auto bg-background` para asegurar que la barra lateral permanezca completamente fija y estática a la izquierda mientras que el contenido principal (como el calendario) se desplaza de forma independiente.
